...
# <a id='dt_86y8HF8yI'>**add_header**</a>:  Add Index entry, RCS ID, copyright, preliminary
...
# <a id='dt_86y8HF8yL'>**anagram**</a>:  Print anagrams of word specified in argument.
**Uses system english dictionary (/usr/share/dict/words).

# <a id='dt_86y8HJL7Q'>**cat_media**</a>:  Concatinates different types of files (movies, pics,
**audio etc).  Complex because videos may not be same aspect ratio,
**audios may have different sample rates, etc.

# <a id='dt_86y8HJL7T'>**copydb**</a>:  Copy data from gdbm or perl objects into another
**format. Easily extensible to copy SQL or other database formats
**#as cpi_db is extended.

# <a id='dt_86y8HJL7W'>**count_domains**</a>:  Read ip log file & count packets by domain
**Information assumed to be on standard in put.

# <a id='dt_86y8HJL7Z'>**descape**</a>:  Remove terminal control escape sequences from stdin.
**Useful for analyzing output from software that thinks it is being
**run interactively.

# <a id='dt_86y8HJL7c'>**doc_sep**</a>:  Filter out all but the specified headers for documentation.
**Can also be used to remove documentation to leave just source

# <a id='dt_86y8HJL7f'>**embed_images**</a>:  Convert image URLs to the data they are pointing to.
**Useful for creating html that doesn't require internet connection.

# <a id='dt_86y8HJL7i'>**fix_header**</a>:  Add Index entry, RCS ID, copyright, preliminary
history & documentation to source.  More complex than at first
blush because it needs to figure out comment convention so that new
header doesn't change semantics of program.

# <a id='dt_86y8HJL7l'>**host_of_ssh**</a>:  Print info about inbound SSH connections.
**Filters output of lsof.

# <a id='dt_86y8HJL7o'>**indent_json**</a>:  Output json in a semi-readable (indented) format.
**Note that there are more standard libraries for doing this
**but the developer wasn't wild about the format.

# <a id='dt_86y8HJL7r'>**merge_scan_batch**</a>:  Collect multiple pages into a single document
...
# <a id='dt_86y8HJL7u'>**mixtape**</a>:  Create music collections, or play randomly from collection
**or simply play random music from a specified directory.

# <a id='dt_86y8HJL7x'>**nene**</a>:  Copy data from any ext to any ext (converting as required)
...
# <a id='dt_86y8HJL80'>**old_javascript**</a>:  Fix javascript to run on old iPads due to syntax
**"for( const x of" and "for( const x in" syntax newer than those
**machines.

# <a id='dt_86y8HJL83'>**radix**</a>:  Find all radices where number has specified digits
**Radices from 2 to 62 checked.

# <a id='dt_86y8HJL86'>**remind**</a>:  Send e-mail reminders based on user's .remind
**Invoked once a day in the wee hours of the morning.

#Remind - Software to send e-mail reminders according to dated entries

# <a id='dt_86y8HJL89'>**rnh**</a>:  Simple file to manipulate old UNH runoff files.

# <a id='dt_86y8HJL8C'>**sanifile**</a>:  Fix filenames to be UNIX friendly and convert to standard types
...
# <a id='dt_86y8HJL8F'>**scan**</a>:  A friendly front end to scanimage to scan documents
**Uses merge_scan_batch and cat_media to construct pdf files.

# <a id='dt_86y8HJL8I'>**screens**</a>:  Obtain info about connected monitors.
**Interfaces with X via xrandr.

# <a id='dt_86y8HJL8L'>**seconds_to_daytime**</a>:  Convert seconds since epoch to readable format

# <a id='dt_86y8HNXHQ'>**send_text**</a>:  Send a text to specified phone by e-mail to special address
**Really just a front end to e-mail.

# <a id='dt_86y8HNXHT'>**setup_access_point**</a>:  Configure local WIFI card to be access point

# <a id='dt_86y8HNXHW'>**show**</a>:  Play specified media on local display/speakers

# <a id='dt_86y8HNXHZ'>**square_icon**</a>:  Create an icon suitable for iPhone based on supplied text

# <a id='dt_86y8HNXHc'>**sudoinstall**</a>:  Try to install something.  I fail, sudo and try again.
...
# <a id='dt_86y8HNXHf'>**sumdir**</a>:  Create a file containing checksums of files in directory

# <a id='dt_86y8HNXHi'>**tmog**</a>:  Type, Mode, Owner and Group - create specified file
...
# <a id='dt_86y8HNXHl'>**unique_name**</a>:  Fill in unique digits to create a filename
**Create a unique name for a file based on % notation in the
**supplied name.

# <a id='dt_86y8HNXHo'>**upend**</a>:  Print lines with text reversed within the line

# <a id='dt_86y8HNXHr'>**words_with**</a>:  Show words using specified letters

# <a id='dt_86y8HNXHu'>**youtube**</a>:  Convenient interface yt-dlp to download youtube videos
